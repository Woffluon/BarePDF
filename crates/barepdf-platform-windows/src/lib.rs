...
pub fn executable_file_version(path: &Path) -> Result<[u16; 4], PdfError> {
    let path = wide_null(path.as_os_str());
    let mut ignored = 0_u32;
    // SAFETY: The path is a live, NUL-terminated UTF-16 string and the output pointer is valid.
    let byte_len = unsafe { GetFileVersionInfoSizeW(path.as_ptr(), &mut ignored) };
    if byte_len == 0 {
        return Err(PdfError::PlatformError(
            "The update has no Windows file version metadata".into(),
        ));
    }
    ...
    let word_len = (byte_len as usize)
        ...
        .ok_or_else(|| PdfError::PlatformError("Version metadata is too large".into()))?;
    let mut buffer = vec![0_usize; word_len];
    // SAFETY: Buffer is writable for byte_len bytes and path remains valid for the call.
    let read =
        unsafe { GetFileVersionInfoW(path.as_ptr(), 0, byte_len, buffer.as_mut_ptr().cast()) };
    if read == 0 {
        return Err(PdfError::PlatformError(
            "Could not read the update's Windows version metadata".into(),
        ));
    }
    ...
    // SAFETY: Buffer contains version data returned by GetFileVersionInfoW; outputs are valid.
    let queried = unsafe {
        VerQueryValueW(
            buffer.as_ptr().cast(),
            root.as_ptr(),
    ...
    if queried == 0 || value.is_null() || value_len < std::mem::size_of::<VS_FIXEDFILEINFO>() as u32
    {
        return Err(PdfError::PlatformError(
            "The update has invalid Windows version metadata".into(),
        ));
    }
    // SAFETY: VerQueryValueW returned a non-null VS_FIXEDFILEINFO-sized root value.
...
pub fn launch_installer(path: &Path) -> Result<(), PdfError> {
    Command::new(path)
        .spawn()
        .map(|_| ())
        .map_err(|error| PdfError::PlatformError(format!("Could not start installer: {error}")))
}
...
pub fn open_url(url: &str) -> Result<(), PdfError> {
